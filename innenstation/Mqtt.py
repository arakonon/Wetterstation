...
class EspAußen:
    def __init__(self, host = "localhost", topic = "esp32/#", timeout = 600):
        ...
        self._cli.loop_start()  # NETZWERK_LOOP IM HINTERGRUND STARTEN
    ...
    def on_message(self, client, userdata, msg):
        ...
        try: 
            ...
            return
        self._values[key] = (val, time.time()) # Speichert Werte mit Key(topic), Wert und Zeit 
    ...
    def is_alive(self):
        if not self._values: # Wenn leer
            return False  # Prüft, ob in letzter Zeit Werte empfangen wurden (Timeout)
        last_timestamp = max(timestamp for _, timestamp in self._values.values())  # Jüngster Zeitstempel aller Werte
        ...
        return time.time() - last_timestamp < self._timeout
...
class MQTTPublisher:
    def __init__(self, topic_prefix="wetterstation/innen", host="localhost"):
        ...
        self._cli.connect(host, 1883, 60) # connect ist also eine Methode von paho-mqtt
